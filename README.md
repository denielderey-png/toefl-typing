...
7. **每日新闻** —— 接入后端后，抓取当天 BBC 新闻（头条 / 世界 / 科技 / 科学 / 商业）摘要作为打字材料，点一条即进入文章模式练习，紧跟时事。
...
「每日新闻」抓取当天 BBC 官方 RSS 摘要作为打字材料。浏览器不能直接跨域抓取，需用 `news-worker.js` 部署一个 Cloudflare Worker：
...
5. 打开网页 →「每日新闻」标签 → 粘贴该地址保存。之后顶部可切换 头条/世界/科技/科学/商业，点任意一条即进入文章模式开练。

> 仅使用 BBC 公开 RSS 的「标题 + 摘要」（供聚合转载用），不抓全文。请遵守来源方条款，仅作个人学习。Worker 已对结果缓存 15 分钟，减轻来源压力。
